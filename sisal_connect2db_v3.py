...
FROM site s
JOIN drip_entity d ON d.site_id = s.site_id
JOIN drip_rate_sample dr ON dr.drip_entity_id = d.drip_entity_id
WHERE s.site_name = :site_name
ORDER BY d.drip_entity_id, dr.drip_rate_start_yyyy, dr.drip_rate_start_mm, dr.drip_rate_start_dd;
"""
...
FROM site s
JOIN site_link_precip slp ON slp.site_id = s.site_id
JOIN precip_site psm ON psm.precip_site_id = slp.precip_site_id
JOIN precip_entity pem ON pem.precip_entity_id = slp.precip_entity_id
JOIN precip_sample p ON p.precip_entity_id = pem.precip_entity_id
WHERE s.site_name = :site_name
ORDER BY pem.precip_entity_id, p.precip_start_yyyy, p.precip_start_mm, p.precip_start_dd;
"""
...
with engine.begin() as conn:
    ...
    global_metadata_df = pd.read_sql(global_metadata_sql, conn)  # optional
...
with pd.ExcelWriter(out_xlsx, engine="openpyxl") as writer:
    ...
    global_metadata_df.to_excel(writer, sheet_name="global_metadata", index=False)  # optional
