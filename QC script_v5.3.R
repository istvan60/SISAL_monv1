...
if (dim(cave_temperature_sample_data)[1] != 0){
  ...
  T07.04_hhmm_check <- check_hhmm_within_range(cave_temperature_sample_data$cave_temperature_hhmm, 1, 2359)
  ...
  cat("HHMM Check: ", T07.04_hhmm_check, "\n")
  ...
  # Summarize all checks
  T07_all_checks <- T07.01_year_check & T07.02_month_check & T07.03_day_check & T07.04_hhmm_check & T07.05_number_check & T07.06_measurement_check & T07.07_precision_check
...
# Pre-calc a date for mod_carb_sample_data
mod_carb_sample_data <- mod_carb_sample_data %>%
  mutate(
    mod_carb_start_yyyy = as.numeric(mod_carb_start_yyyy),
    mod_carb_start_mm   = as.numeric(mod_carb_start_mm),
    mod_carb_start_dd   = as.numeric(mod_carb_start_dd),
    date                = concat_date(mod_carb_start_yyyy, mod_carb_start_mm, mod_carb_start_dd)
  ) %>%
  filter(!is.na(date))

# Start PDF
pdf_path <- paste0("graphics_output_", cave_name, ".pdf")
pdf(pdf_path)

# -----------------------------------------------------
# Plot 1.1: World map
site_data            <- read.xlsx(file_name, sheet="site", startRow=2, colNames=TRUE)
precip_site_metadata <- read.xlsx(file_name, sheet="precip_site_metadata",
...
fig_map_world <- function(points, scale) {
  ...
  ggplot() +
    ...
    geom_point(data = points, aes(x = lon, y = lat),
               color = "red", size = 3) +
    geom_text(data = points, aes(x = lon, y = lat, label = name),
              vjust = -1, hjust = 0.5) +
    coord_fixed(ratio = 1.3,
                xlim = scale[1:2],
                ylim = scale[3:4]) +
    labs(title = "Site and Precipitation Locations",
...
fig_map_zoomed <- function(points) {
  world_map <- map_data("world")
  lon_rng <- range(points$lon, na.rm=TRUE) + c(-5,5)
  lat_rng <- range(points$lat, na.rm=TRUE) + c(-5,5)
  ggplot() +
    geom_polygon(data = world_map,
                 aes(x = long, y = lat, group = group),
                 fill = "lightblue", color = "black") +
    ...
    geom_text(data = points, aes(x = lon, y = lat, label = name),
              vjust = -1, hjust = 0.5) +
    coord_fixed(ratio = 1.3, xlim = lon_rng, ylim = lat_rng) +
    labs(title = "Zoomed-In Map of Site and Precipitation",
         x = "Longitude", y = "Latitude") +
    theme_minimal()
}
...
# -----------------------------------------------------
# Plot 2: Precip amount vs accumulation time (no date issues)
ggplot(precip_sample_data, aes(x = precip_accumulation_time,
                               y = precip_amount,
                               color = precip_accumulation_unit)) +
  geom_point(size=3, alpha=0.8) +
  labs(title = "Precipitation Amount vs Accumulation Time",
       x = "Accumulation Time", y = "Amount",
       color = "Unit") +
  theme_minimal()
...
# -----------------------------------------------------
# Plot 3: Precipitation d18O vs d2H measurements (with regression line)
#Ensure mod_p and mod_d are defined first

# 1. Coerce to numeric & filter complete cases
precip_clean <- precip_sample_data %>%
  mutate(
    precip_d18O_measurement = as.numeric(precip_d18O_measurement),
    precip_d2H_measurement  = as.numeric(precip_d2H_measurement)
  ) %>%
  filter(!is.na(precip_d18O_measurement),
         !is.na(precip_d2H_measurement))

drip_clean <- drip_iso_sample_data %>%
  mutate(
    drip_d18O_measurement = as.numeric(drip_d18O_measurement),
    drip_d2H_measurement  = as.numeric(drip_d2H_measurement)
  ) %>%
  filter(!is.na(drip_d18O_measurement),
         !is.na(drip_d2H_measurement))

# 2. Only fit models when there are at least 2 points
if (nrow(precip_clean) >= 2 && nrow(drip_clean) >= 2) {
  
  mod_p <- lm(precip_d18O_measurement ~ precip_d2H_measurement,
              data = precip_clean)
  mod_d <- lm(drip_d18O_measurement   ~ drip_d2H_measurement,
              data = drip_clean)
  
  eq_p <- sprintf("Precip: y = %.2f x + %.2f, R² = %.2f",
                  coef(mod_p)["precip_d2H_measurement"],
                  coef(mod_p)["(Intercept)"],
                  summary(mod_p)$r.squared)
  eq_d <- sprintf("Drip:   y = %.2f x + %.2f, R² = %.2f",
                  coef(mod_d)["drip_d2H_measurement"],
                  coef(mod_d)["(Intercept)"],
                  summary(mod_d)$r.squared)
  
  combined <- bind_rows(
    precip_clean %>% transmute(d18O = precip_d18O_measurement,
                               d2H  = precip_d2H_measurement,
                               group = "Precipitation"),
    drip_clean   %>% transmute(d18O = drip_d18O_measurement,
                               d2H  = drip_d2H_measurement,
                               group = "Drip Isotope")
  )
  
  ggplot(combined, aes(x = d18O, y = d2H, color = group)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE) +
    geom_abline(intercept = 10, slope = 8, linetype = "dashed") +
    annotate("text",
             x = min(combined$d18O, na.rm = TRUE),
             y = max(combined$d2H, na.rm = TRUE) * 1.05,
             label = eq_p, hjust = 0, color = "blue") +
    annotate("text",
             x = min(combined$d18O, na.rm = TRUE),
             y = max(combined$d2H, na.rm = TRUE),
             label = eq_d, hjust = 0, color = "red") +
    annotate("text",
             x = max(combined$d18O, na.rm = TRUE) * 1.1,
             y = max(combined$d2H, na.rm = TRUE) * 1.05,
             label = "GMWL (y = 8x + 10)", hjust = 0) +
    labs(title = "d¹⁸O vs d²H with Linear Fits & GMWL",
         x = "δ¹⁸O", y = "δ²H") +
    theme_minimal()
  ...
  plot.new()
  text(0.5, 0.5,
       "Not enough non-NA δ¹⁸O/δ²H pairs to fit regression",
       cex = 1.2, col = "red", adj = 0.5)
}

# -----------------------------------------------------
# Plot 4: δ²H vs start date
if (nrow(precip_sample_data)==0) {
  plot.new()
  text(0.5,0.5,"No data available in precip_sample_data",adj=0.5)
} else {
  df4 <- precip_sample_data %>%
    mutate(
      precip_start_yyyy      = as.numeric(precip_start_yyyy),
      precip_start_mm        = as.numeric(precip_start_mm),
      precip_start_dd        = as.numeric(precip_start_dd),
      precip_d2H_measurement = as.numeric(precip_d2H_measurement),
      date = concat_date(precip_start_yyyy,precip_start_mm,precip_start_dd)
    ) %>%
    filter(!is.na(date), !is.na(precip_d2H_measurement))
  if (nrow(df4)>0) {
    ggplot(df4, aes(x=date,y=precip_d2H_measurement,color=precip_entity_name))+
      geom_line()+geom_point()+
      labs(title="δ²H vs Start Date",
           x="Date",y="δ²H Measurement",color="Entity")+theme_minimal()
  } else {
    plot.new(); text(0.5,0.5,"No valid date+δ²H pairs to plot",col="red",adj=0.5)
  }
}

# -----------------------------------------------------
# Plot 5: δ¹⁸O vs start date
if (nrow(precip_sample_data)==0) {
  plot.new()
  text(0.5,0.5,"No data available in precip_sample_data",adj=0.5)
} else {
  df5 <- precip_sample_data %>%
    mutate(
      precip_start_yyyy       = as.numeric(precip_start_yyyy),
      precip_start_mm         = as.numeric(precip_start_mm),
      precip_start_dd         = as.numeric(precip_start_dd),
    ...
    filter(!is.na(date), !is.na(precip_d18O_measurement))
  if (nrow(df5)>0) {
    ggplot(df5,aes(x=date,y=precip_d18O_measurement,color=precip_entity_name))+
      geom_line()+geom_point()+
      labs(title="δ¹⁸O vs Start Date",
           x="Date",y="δ¹⁸O Measurement",color="Entity")+theme_minimal()
  } else {
    plot.new(); text(0.5,0.5,"No valid date+δ¹⁸O pairs to plot",col="red",adj=0.5)
  }
}

# -----------------------------------------------------
# Plot 6: Cave temperature vs date
if (nrow(cave_temperature_sample_data)==0) {
  plot.new(); text(0.5,0.5,"No data in cave_temperature_sample_data",adj=0.5)
} else {
  df6 <- cave_temperature_sample_data %>%
    mutate(
      cave_temperature_yyyy       = as.numeric(cave_temperature_yyyy),
      cave_temperature_mm         = as.numeric(cave_temperature_mm),
      cave_temperature_dd         = as.numeric(cave_temperature_dd),
      cave_temperature_measurement= as.numeric(cave_temperature_measurement),
      date = concat_date(cave_temperature_yyyy,cave_temperature_mm,cave_temperature_dd)
    ) %>%
    filter(!is.na(date),!is.na(cave_temperature_measurement))
  if (nrow(df6)>0) {
    ggplot(df6,aes(x=date,y=cave_temperature_measurement,color=cave_entity_name))+
      geom_line()+geom_point()+
      labs(title="Cave Temperature vs Date",
           x="Date",y="Temperature",color="Entity")+theme_minimal()
  } else {
    plot.new(); text(0.5,0.5,"No valid date+temp pairs",col="red",adj=0.5)
  }
}

# -----------------------------------------------------
# Plot 7: Cave RH vs date
if (nrow(cave_relative_humidity_sample_data)==0) {
  plot.new(); text(0.5,0.5,"No data in cave_relative_humidity_sample_data",adj=0.5)
} else {
  df7 <- cave_relative_humidity_sample_data %>%
    mutate(
      cave_relative_humidity_yyyy       = as.numeric(cave_relative_humidity_yyyy),
      cave_relative_humidity_mm         = as.numeric(cave_relative_humidity_mm),
      cave_relative_humidity_dd         = as.numeric(cave_relative_humidity_dd),
      cave_relative_humidity_measurement= as.numeric(cave_relative_humidity_measurement),
      date = concat_date(cave_relative_humidity_yyyy,
                         cave_relative_humidity_mm,
                         cave_relative_humidity_dd)
    ) %>%
    filter(!is.na(date),!is.na(cave_relative_humidity_measurement))
  if (nrow(df7)>0) {
    ggplot(df7,aes(x=date,y=cave_relative_humidity_measurement,
                   color=cave_entity_name))+
      geom_line()+geom_point()+
      labs(title="Cave RH vs Date",
           x="Date",y="Relative Humidity",color="Entity")+theme_minimal()
  } else {
    plot.new(); text(0.5,0.5,"No valid date+RH pairs",col="red",adj=0.5)
  }
}

# -----------------------------------------------------
# Plot 8: Cave pCO2 vs date
if (nrow(cave_pCO2_sample_data)==0) {
  plot.new(); text(0.5,0.5,"No data in cave_pCO2_sample_data",adj=0.5)
} else {
  df8 <- cave_pCO2_sample_data %>%
    mutate(
      cave_pCO2_yyyy       = as.numeric(cave_pCO2_yyyy),
      cave_pCO2_mm         = as.numeric(cave_pCO2_mm),
      cave_pCO2_dd         = as.numeric(cave_pCO2_dd),
      cave_pCO2_measurement= as.numeric(cave_pCO2_measurement),
      date = concat_date(cave_pCO2_yyyy,cave_pCO2_mm,cave_pCO2_dd)
    ) %>%
    filter(!is.na(date),!is.na(cave_pCO2_measurement))
  if (nrow(df8)>0) {
    ggplot(df8,aes(x=date,y=cave_pCO2_measurement,color=cave_entity_name))+
      geom_line()+geom_point()+
      labs(title="Cave pCO₂ vs Date",
           x="Date",y="pCO₂",color="Entity")+theme_minimal()
  } else {
    plot.new(); text(0.5,0.5,"No valid date+pCO₂ pairs",col="red",adj=0.5)
  }
}

# -----------------------------------------------------
# Plot 9: Drip δ¹⁸O vs date
if (nrow(drip_iso_sample_data)==0) {
  plot.new(); text(0.5,0.5,"No data in drip_iso_sample_data",adj=0.5)
} else {
  # reuse df9 from above if you like, or rebuild exactly like df7...
  df9 <- drip_iso_sample_data %>%
    mutate(
      drip_iso_start_yyyy     = as.numeric(drip_iso_start_yyyy),
      drip_iso_start_mm       = as.numeric(drip_iso_start_mm),
      drip_iso_start_dd       = as.numeric(drip_iso_start_dd),
      drip_d18O_measurement   = as.numeric(drip_d18O_measurement),
      date = concat_date(drip_iso_start_yyyy,drip_iso_start_mm,drip_iso_start_dd)
    ) %>%
    filter(!is.na(date), !is.na(drip_d18O_measurement))
  if (nrow(df9)>0) {
    ggplot(df9,aes(x=date,y=drip_d18O_measurement,color=drip_entity_name))+
      geom_line()+geom_point()+
      labs(title="Drip δ¹⁸O vs Date",
           x="Date",y="δ¹⁸O",color="Entity")+theme_minimal()
  } else {
    plot.new(); text(0.5,0.5,"No valid date+δ¹⁸O pairs",col="red",adj=0.5)
  }
}

# -----------------------------------------------------
# Plot 10: Drip δ²H vs date
if (nrow(drip_iso_sample_data)==0) {
  plot.new(); text(0.5,0.5,"No data in drip_iso_sample_data",adj=0.5)
} else {
  df10 <- drip_iso_sample_data %>%
    mutate(
      drip_iso_start_yyyy   = as.numeric(drip_iso_start_yyyy),
      drip_iso_start_mm     = as.numeric(drip_iso_start_mm),
      drip_iso_start_dd     = as.numeric(drip_iso_start_dd),
    ...
    filter(!is.na(date), !is.na(drip_d2H_measurement))
  if (nrow(df10)>0) {
    ggplot(df10,aes(x=date,y=drip_d2H_measurement,color=drip_entity_name))+
      geom_line()+geom_point()+
      labs(title="Drip δ²H vs Date",
           x="Date",y="δ²H",color="Entity")+theme_minimal()
  } else {
    plot.new(); text(0.5,0.5,"No valid date+δ²H pairs",col="red",adj=0.5)
  }
}

# -----------------------------------------------------
# Plot 11: Drip rate vs date
if (nrow(drip_rate_sample_data)==0) {
  plot.new(); text(0.5,0.5,"No data in drip_rate_sample_data",adj=0.5)
} else {
  df11 <- drip_rate_sample_data %>%
    mutate(
      drip_rate_start_yyyy  = as.numeric(drip_rate_start_yyyy),
      drip_rate_start_mm    = as.numeric(drip_rate_start_mm),
      drip_rate_start_dd    = as.numeric(drip_rate_start_dd),
      drip_rate_measurement = as.numeric(drip_rate_measurement),
      date = concat_date(drip_rate_start_yyyy,drip_rate_start_mm,drip_rate_start_dd)
    ) %>%
    filter(!is.na(date), !is.na(drip_rate_measurement))
  if (nrow(df11)>0) {
    ggplot(df11,aes(x=date,y=drip_rate_measurement,color=drip_entity_name))+
      geom_line()+geom_point()+
      labs(title="Drip Rate vs Date",
           x="Date",y="Rate",color="Entity")+theme_minimal()
  } else {
    plot.new(); text(0.5,0.5,"No valid date+rate pairs",col="red",adj=0.5)
  }
}

# -----------------------------------------------------
# Plot 12: Mod_carb δ¹⁸O vs date
if (nrow(mod_carb_sample_data)==0) {
  plot.new(); text(0.5,0.5,"No data in mod_carb_sample_data",adj=0.5)
} else {
  df12 <- mod_carb_sample_data %>%
    mutate(mod_carb_d18O_measurement = as.numeric(mod_carb_d18O_measurement)) %>%
    filter(!is.na(date), !is.na(mod_carb_d18O_measurement))
  if (nrow(df12)>0) {
    ggplot(df12,aes(x=date,y=mod_carb_d18O_measurement,color=drip_entity_name))+
      geom_line()+geom_point()+
      labs(title="Mod-Carb δ¹⁸O vs Date",
           x="Date",y="δ¹⁸O",color="Entity")+theme_minimal()
  } else {
    plot.new(); text(0.5,0.5,"No valid date+δ¹⁸O pairs",col="red",adj=0.5)
  }
...
# -----------------------------------------------------
# Plot 13: Mod_carb δ¹³C vs date
if (nrow(mod_carb_sample_data)==0) {
  plot.new(); text(0.5,0.5,"No data in mod_carb_sample_data",adj=0.5)
} else {
  df13 <- mod_carb_sample_data %>%
    mutate(mod_carb_d13C_measurement = as.numeric(mod_carb_d13C_measurement)) %>%
    filter(!is.na(date), !is.na(mod_carb_d13C_measurement))
  if (nrow(df13)>0) {
    ggplot(df13,aes(x=date,y=mod_carb_d13C_measurement,color=drip_entity_name))+
      geom_line()+geom_point()+
      labs(title="Mod-Carb δ¹³C vs Date",
           x="Date",y="δ¹³C",color="Entity")+theme_minimal()
  } else {
    plot.new(); text(0.5,0.5,"No valid date+δ¹³C pairs",col="red",adj=0.5)
  }
}
